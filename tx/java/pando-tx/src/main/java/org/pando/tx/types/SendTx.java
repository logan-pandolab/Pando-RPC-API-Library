...
public final class SendTx implements Tx {
    ...
    public SendTx(byte[] senderAddr, byte[] receiverAddr, BigInteger pandoWei, BigInteger ptxWei, 
        BigInteger feeInPTXWei, long senderSequence) {
        this.fee = new Coins(BigInteger.valueOf(0), feeInPTXWei);

        TxInput txInput = new TxInput(senderAddr, pandoWei, ptxWei.add(feeInPTXWei), senderSequence);
        this.inputs = new TxInput[]{txInput};

        TxOutput txOutput = new TxOutput(receiverAddr, pandoWei, ptxWei);
