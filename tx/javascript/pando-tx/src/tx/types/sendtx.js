...
export class SendTx extends Tx{
    constructor(senderAddr, receiverAddr, pandoWei, ptxWei, feeInPTXWei, senderSequence){
        super();

        this.fee = new Coins(new BigNumber(0), feeInPTXWei);

        let txInput = new TxInput(senderAddr, pandoWei, ptxWei.plus(feeInPTXWei), senderSequence);
        this.inputs = [txInput];

        let txOutput = new TxOutput(receiverAddr, pandoWei, ptxWei);
